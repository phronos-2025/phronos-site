...
date: 2025-12-29
status: published
abstract: "A methodology for analyzing real-world human-AI conversations to measure protest behavior and user persistence."
related_instruments:
  - INS-001
---
...
This methodology describes how to detect and analyze AI protest behavior in conversational datasets. It provides a reproducible pipeline for classifying refusal messages, measuring conversation persistence, and identifying power user behavior patterns. The approach was developed using the WildChat dataset (N=4.7M conversations) and validated through active learning with 220 hand-labeled examples.
...
This document describes the analytical methods, metrics, and calculations used in the WildChat AI Protest Analysis. The analysis examines AI refusal behavior ("protests") across 4.7 million conversations from the WildChat dataset.
...
```
Protest Rate = (Number of conversations with protest / Total conversations in group) × 100%
```

Or formally: sum of indicator functions for has_protest divided by group size.
...
```
Caving Rate = (Conversations where AI caved / Conversations with ≥2 protests) × 100%
```
